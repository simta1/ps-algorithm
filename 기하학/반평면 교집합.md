...
#### $O(N^2 \log{N})$ 보로노이 다이어그램 구하는 법
```cpp
vector<Point<T> > border(m), points(n); // 이미 주어져있는 상황
// border의 점들이 반시계방향으로 정렬되어 주어져있는 경우라 가정

for (int i = 0; i < n; i++) {
    auto [x1, y1] = points[i];

    vector<HalfPlane<true> > planes; // border의 점들이 반시계방향 정렬이므로
    for (int i = 0, j = border.size() - 1; i < border.size(); j = i++) planes.emplace_back(border[j], border[i]);
    ...
    // ex) 각 영역의 넓이 구해야 되면
    // area[i] = getPolygonAreaDouble(HPI(planes)) / 2;
}
```

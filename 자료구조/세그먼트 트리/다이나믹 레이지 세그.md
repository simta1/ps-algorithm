...
> updateChange를 사용하는 문제의 경우 대부분 change를 0또는 1로만 하는 문제가 많은데, 이 경우에는 따로 lazy의 자료형을 bool로 바꾸는 게 좋음. 지금은 lazy와 val의 자료형을 무조건 일치시키도록 템플릿을 짜둔지라 직접 코드 바꿔서 사용해야 됨.
```cpp
struct __attribute__((__packed__)) Node {
    T val;
    int l, r;
